...
/**
 * show_error - write an error when no founds a file or directory
 * @name: name of executable
 * @buffer: address of command searched to execute and no found
 * @nerrors: Number of the error
 */
void show_error(char *name, char **buffer, size_t nerrors)
{
	char *n = convert_n(nerrors);
...
void shell_loop(char **argv)
{
	...
	while (1)
	{
		...
		if (child == 0)
		{
			if (execute(buffer) == -1)
			{
				show_error(argv[0], &buffer, errors);
				free(buffer);
				exit(127);
			}
			exit(EXIT_SUCCESS);
