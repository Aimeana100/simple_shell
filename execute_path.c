#include "shell_h.h"
/**
 * deal_with_input_args - create pointer to pointer of options
 * of command to execute
 * @user_input: Command to catch
 * Return: Pointer to pointer of options
 */
char **deal_with_input_args(char *user_input)
{
	int i = 0, j = 0;
	char *path = user_input;
	char *tmpexe = _strdup(user_input);
	char **argv = NULL;
...
/**
 * execute_dir_path - execute the command searching in each directory
 * of PATH variable
 * @name: command to execute
 * @options: options to execute with command
 * Return: 0 is success or -1 is error
 */
int execute_dir_path(char *name, char **options)
{
	DIR *dir;
	int st = 0;
